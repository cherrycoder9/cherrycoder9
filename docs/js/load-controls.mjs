...
const controlsFilePath = '/_includes/controls.html'; // <--- 네 파일 경로로 수정 필수!
/**
 * 페이지 컨트롤 UI를 비동기적으로 로드하고 페이지에 삽입하는 함수
 * @returns {Promise<void>}
 */
async function loadPageControls() {
    // 컨트롤 UI를 삽입할 대상 요소를 ID로 찾기
    // 이 ID를 가진 요소가 네 HTML 어딘가에 있어야 함. 예를 들어 body 맨 끝 같은 곳.
    const controlsContainer = document.getElementById('page-controls-container'); // <--- 이 ID도 네 HTML 구조에 맞게!
    ...
    try {
        ...
        const response = await fetch(controlsFilePath);
        // 응답 실패하면 에러 던지기. 
        if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', loadPageControls);
export {};
